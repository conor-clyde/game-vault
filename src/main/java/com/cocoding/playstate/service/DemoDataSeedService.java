...
/**
 * Seeds a shared demo account and a varied collection (playthroughs, notes, reviews, play logs) using
 * live IGDB payloads so covers and metadata match production API data.
 */
@Service
public class DemoDataSeedService {
    ...
    private static final List<String> DEMO_API_IDS =
            List.of("119133", "194267", "1877", "13577", "18472", "1068", "19560");
    ...
    public void seedIfEnabled() {
        ...
        if (demoUsername == null || demoUsername.isBlank() || demoPassword == null || demoPassword.length() < 8) {
            logger.warn("Demo seed skipped: configure demo.username and demo.password (min 8 characters).");
            return;
        }
        ...
        for (long igdbId : DEMO_GAME_IDS) {
            ...
            UserGame ug = userGameRepository.findByUserIdAndGameId(username, game.getId()).orElseGet(
                    () -> new UserGame(username, game.getId()));
    ...
    private static void applyOwnership(UserGame ug) {
    ...
    private static void applyElden(UserGame ug) {
        ...
        ug.setNotes(
                "Taking it slow: I clear every cave before moving the story. The world feels endless in the best way.");
        ...
        ug.setReflectionHighlight("Level design rewards curiosity without waypoints holding your hand.");
        ug.setReflectionTagsJson(ReflectionTagsJson.serializeList(List.of("open-world", "tough-but-fair", "co-op-summons")));
    }
    ...
    private static void applyBg3(UserGame ug) {
        ...
        ug.setReview(
                "Larian's writing respects player agency without losing heart. Combat stayed fresh through Act III, "
                        + "and I actually cared about companion quests. Minor pacing dips in the city act, but the highs "
                        + "more than compensate.");
        ug.setWhyPlaying(WhyPlaying.STORY);
        ug.setReflectionTagsJson(ReflectionTagsJson.serializeList(List.of("co-op", "turn-based", "branching-story")));
    ...
    private static void applyCyberpunk(UserGame ug) {
        ...
        ug.setReview(
                "Night City is still the star. Main story landed better post-2.0; a few rough quest beats remain.");
    ...
    private static void applyStardew(UserGame ug) {
        ...
        ug.setReflectionTagsJson(ReflectionTagsJson.serializeList(List.of("cozy", "farming", "pixel-art")));
    ...
    private static void applyHollowKnight(UserGame ug) {
        ...
        ug.setReview(
                "Every new ability recontextualizes old zones. Bosses teach patterns; deaths feel like tuition, not punishment.");
    ...
    private static void applyGodOfWar(UserGame ug) {
        ...
        ug.setReview("Character work carries the middle acts; optional realms padded the runtime a bit.");
