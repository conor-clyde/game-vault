...
import com.cocoding.playstate.service.IgdbTokenService;
...
public class DemoEnvironmentRunner implements ApplicationRunner {
    ...
    public DemoEnvironmentRunner(
            DemoDataSeedService demoDataSeedService, IgdbTokenService igdbTokenService, Environment environment) {
    ...
    private void validateLocalIgdbCredentials() {
        if (isBlank(igdbClientId) || isBlank(igdbClientSecret)) {
            logger.warn(
                    "Local profile started without IGDB credentials. Set igdb.client.id and igdb.client.secret in application-local.properties.");
            return;
        }
        try {
